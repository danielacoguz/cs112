...
void LinkedQueueTester::testAssignment() {
	...
	assert( !q1.isFull() );
	assert( q1.getSize() == 0 );
	assert( q1.myFirstPtr == nullptr );
	assert( q1.myLastPtr == nullptr );
	cout << " 1 " << flush;
	//non-empty to empty
	LinkedQueue q3;
	LinkedQueue q4;
	...
	assert( q5.isEmpty() );
	assert( q5.getSize() == 0 );
	assert( q5.myFirstPtr == nullptr );
	assert( q5.myLastPtr == nullptr );
	cout << " 3 " << flush;
...
void LinkedQueueTester::testCopyConstructor() {
	...
	assert( q2.isEmpty() );
	assert( !q2.isFull() );
	assert( q2.myFirstPtr == nullptr );
	assert( q2.myLastPtr == nullptr );
	assert( q2.getSize() == 0 );
	cout << " 1 " << flush;
	// queue of 3 items
	LinkedQueue q3;
